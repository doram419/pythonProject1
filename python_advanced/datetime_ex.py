# import datetime
from datetime import datetime, date, time # 특정 패키지 내부의 객체 import 가능
...
def get_datetime():
    ...
    print("1900년대의 마지막 날:", dt)

    # 없는 날짜를 지정하면 ValueError
    ...
    # time 객체는 datetime 객체의 시관 관련 속성, 메서드만 사용 가능

def timedelta_ex():
    """
    timedelta 클래스 : 두 날짜 사이의 시간 간격을 나타냄
    """
    current = datetime.now() # 현재 시간
    past = datetime(2012, 9, 24) # 과거
    # 대소 비교 가능
    print(current > past) # current가 past보다 큰 가(뒤인가?)
    # 두 날짜의 간격
    diff = current - past
    print("시간 간격:", diff)
